<template>
  <div>
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                ...
                <li @click="addAddress" class="addr-new">
    ...
    <modal :mdShow="isAddMdShow" @close="closeModal">
      <ul slot="message">
        <li>姓名：<input type="text" name="" v-model="address.userName"></li>
        <li>地址：<input type="text" name="" v-model="address.streetName"></li>
        <li>电话：<input type="text" name="" v-model="address.tel"></li>
        <li>邮编：<input type="text" name="" v-model="address.postCode"></li>
        <li>是否设置为默认地址: <input type="checkbox" name="" v-model="address.isDefault"> </li>
      </ul>
      <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="addAddressConfirm">确认</a>
  ...
  export default {
    ...
    methods: {
      ...
      delAddress () {
        ...
        this.$http.post('/users/delAddress', {addressId})
        .then(res => {
          console.log(res)
          if (res.data.status === '0') {
            this.closeModal()
            // 这里会请求一次ajax，优化的话删除数组中对应的地址对象
            this.init()
          }
          this.isMdShow = false
        })
