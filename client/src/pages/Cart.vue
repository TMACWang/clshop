...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 编辑商品类
      // 改变商品项的数量或和选中状态，并实时请求api保存到数据库中
      // 如果请求成功时，判断如果是改变数量，就更新一下vuex中的数量
      editCart (flag, item) {
        if (flag === 'add') {
          item.productNum++
        } else if (flag === 'minu') {
      ...
      // 得到准备删除商品项的商品ID，
      // 请求api删除数据，
      // 并且更新页面内容和购物车的数量，减去该商品类的所有商品的数量
      delCart () {
        let productId = this.delItem.productId
        this.$http.post('/users/cartDel', {productId})
        .then(res => {
          console.log(res)
      ...
      totalPrice () {
        ...
        this.cartList.forEach(item => {
          ...
          if (parseInt(item.checked) === 1) {
            checkedPrice += parseFloat(item.salePrice) * parseInt(item.productNum)
          }
        })
        return checkedPrice
      },
      // 被选中的商品项的数量
      // 遍历checked属性为'1'的商品项，得到总数
      checkedCount () {
        let count = 0
        this.cartList.forEach(item => {
